...
exports.addToCart = asyncErrorHandler(async (req, res, next) => {
    ...
    if (!productId || !quantity || !sellerId) {
        return next(new handleError("Please provide product, quantity and seller", 400));
    }

    const cart = await CartService.addItemToCart(req.user._id, productId, quantity, sellerId);
